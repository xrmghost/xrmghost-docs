...
`env edit` is a **Pro-tier** feature. Ensure your licence is active and at the Pro level before running this command. Use [`xg license status`](../reference/license-status.mdx) to verify.
:::
...
`env edit` always launches an interactive wizard — it cannot be driven entirely from CLI flags in the current version. To automate environment management, consider exporting the configuration with [`env export`](./env-export.mdx), editing the JSON file, and re-importing it with [`env import`](./env-import.mdx).
...
- [`env list`](./env-list.mdx) — list all registered environments and see which one is active
- [`env add`](./env-add.mdx) — add a new environment configuration using the same interactive wizard
- [`env delete`](./env-delete.mdx) — remove a registered environment configuration
- [CLI Overview](../index.mdx) — overview of the `xrmghost` CLI and its command groups
