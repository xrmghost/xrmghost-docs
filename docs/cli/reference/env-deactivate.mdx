...
- [`xg env activate`](./env-activate.mdx) — Set a specific Dataverse environment as the active one.
- [`xg env list`](./env-list.mdx) — List all registered Dataverse environments and see which one is currently active.
- [`xg env status`](./env-status.mdx) — Show the active environment URL and name.
